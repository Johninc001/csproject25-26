...
def createinvoice():
    ...
    with ui.column().classes("w-full items-center"):
        ...
        with ui.card().classes("w-85 items-center justify-center").style("box-shadow: 5px 5px 15px 0px rgba(255,255,240, 0.625);"):
            ...
            purchases_label = ui.label("current purchases: none")
            def add_purchase(sel, label):
                val = sel.value
                if val:
                    purchase_list.append(val)
                    label.set_text(", ".join(purchase_list))
            ui.label("select a purchase and click \"Add to list\"")
            purchases = ui.select(purchase_options, with_input=True,value="medicine").style("box-shadow: 2px 2px 5px 0px rgba(100,123,238, 0.625);").props("outlined")
            ui.button("Add to list", on_click=lambda: add_purchase(purchases, purchases_label)).classes("btn w-30")
            ui.button("Make Invoice", color="black", on_click=lambda:handle_invoice_submit(custid.value, fname.value, sname.value, pnum.value, email.value, creditcard.value, expirydate.value, ",".join(purchase_list))).classes("btn w-50 text-white rounded-lg")
